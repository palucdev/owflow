...
Internal skill that manages documentation file operations in `.owflow/docs/`. Not directly user-invocable — called by `flow-init`, `standards-update`, and `standards-discover` skills.
...
- **Project documentation** (source of truth): `.owflow/docs/` in the project root
...
Use this when a project doesn't have `.owflow/docs/` or needs documentation for the first time. This is a **one-time baseline setup** that gives the project a starting point.
...
1. Check if `.owflow/docs/` exists in the project root
...
3. Create the directory structure based on standards_selection:
   ...
   ```
4. Copy standards to the project's `.owflow/docs/standards/` directory. **Source selection**: If `standards_source_path` is provided, copy from that external path. Otherwise, copy from this skill's bundled `docs/standards/` directory:
...
- `standards_source_path` (optional, string): Absolute path to an external standards directory (e.g., `/path/to/other-project/.owflow/docs/standards/`)
...
**Result:** The project now has baseline documentation in `.owflow/docs/`, a comprehensive INDEX.md, and AGENTS.md integration that ensures AI assistance is documentation-aware. Only selected standard categories are initialized.
...
1. Scan the `.owflow/docs/` directory structure
...
5. Write the generated INDEX.md to `.owflow/docs/INDEX.md`
...
2. If adding from plugin baseline:
   ...
   - Copy it to the appropriate location in `.owflow/docs/`
...
2. Check if the documentation exists in `.owflow/docs/`
...
1. Compare the documentation in this skill's bundled `docs/` directory with the project's `.owflow/docs/` directory to identify differences
...
6. If the user chooses to copy any documentation:
   - Copy the selected files from this skill's bundled `docs/` directory to the project's `.owflow/docs/` directory
...
2. For each bundled document:
   ...
   - Check if it exists in the project at `.owflow/docs/[category]/[name].md`
...
7. If `.owflow/docs/` doesn't exist, suggest running the initialization operation first
...
1. **Check structure:**
   - Verify `.owflow/docs/` directory exists
   - Verify all expected subdirectories exist (project/, standards/global/, etc.)
2. **Check INDEX.md:**
   - Verify it exists and is readable
   - Check that all files in `.owflow/docs/` are listed in INDEX.md
...
3. **Check AGENTS.md integration:**
   ...
   - Verify it uses valid file reference format: @.owflow/docs/INDEX.md (with @ prefix, without backticks)
   - Warn if using incorrect formats like `.owflow/docs/INDEX.md` or `@.owflow/docs/INDEX.md` (backticks)
