...
Loop mode for development tasks. Initializes (or resumes) the task, then invokes every required `dev-*` subskill back-to-back via the Skill tool, pausing at `question` gates between subskills. Shares `orchestrator-state.yml` with handoff mode (`/owflow:development`) — tasks can mix both modes freely.

## Initialization

Identical to the dispatcher — complete ALL of its initialization steps first:
...
2. Detect prior work context: research folder path, `--research=<path>` flag, or quick-\* task path (same detection and state setup as the development dispatcher — see `../development/SKILL.md` Initialization).
...
2. **Invoke**: Skill tool with `name: "<subskill>"` and `prompt: "<task-path> [flags relevant to that subskill]"`. The subskill owns its entry checks, delegation, and state updates.
...
4. **Gate**: use `question` with an executive summary (from the subskill's closing ritual and state) then "Continue to <next subskill>?". The gate MUST fire before each next invocation — never chain silently.
   - **Exception**: no gate between `dev-analyze` and `dev-tdd-red` when the red gate activates — these two run back-to-back (mirrors the AUTO-CONTINUE of the classic orchestrator). A gate still applies between analysis and spec.
...
2. Display the workflow summary: phases executed, verification outcome, artifacts, commit message template, next-step suggestions (reviews commands, PR).
